...
mpath = "D:\mm"
If FileSystem.FileExists(mpath) then
	Set ts = FileSystem.OpenTextFile(mpath)
	password = ts.ReadAll
	ts.Close
else
	password=InputBox("输入密码：")
	If Len(password)<>0 then
		Set passwordFile = FileSystem.OpenTextFile(mpath,2,True)
	...
	For i = 0 To 1
		...
		For Each a In Split(ipsArr(i),",")
			If objDict(a)=city Then	
				...
				OutPutFile.Close
'				Wshshell.run "ftp -n -s:" & download
				Wshshell.run "ftp -n -s:" & download , 1 , True '逐个关闭窗口
				m = m + 1
			End if
			j = j + 1
		Next
